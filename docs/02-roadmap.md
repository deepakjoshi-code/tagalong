...
|---|---|---|
| App | **v0.1.0 exists and works.** Every screen in the design spec's IA, both transports, the full content engine, connection hygiene (time on connect, reboot recovery, background disconnect), local‑voice‑only previews, storage persistence, an off‑origin build scanner, unit tests, Playwright smoke and screenshots, ≈ 212 KB gz, zero third‑party requests | The app is *ahead* of the plan. The October–November app columns below are hardening and firmware integration, not construction. The remaining app work is enumerated in PRD §5.9 and is small: 7 P0 items, all of which need firmware to exist |
...
| 0.5 | Oct 2026 | Internal | First pairing against the P0 rig; connection state; pairing‑flag chooser filter; off‑origin scanner enforced in CI |
| 0.7 | Nov 2026 | Internal | Tag‑voice preview; Attached‑to / For; mute over the wire; config read‑back + retry; buffered‑frame timestamps and dedupe |
...
| Preview voices resolve to cloud TTS on some Android builds | Closed in code | Would have undermined the zero‑network claim, which is the whole positioning | The app now speaks only through `localService` voices and says so when a device has none (PRD X‑07). Founder acknowledgement at G0 that silent previews on some phones are the accepted cost (PRD §14 Q6) |
